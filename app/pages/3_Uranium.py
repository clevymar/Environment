...
fig1 = px.bar(df.loc["Total world"].T, title="Annual Uranium production").update_layout(xaxis_title="Year", yaxis_title="Tonnes")

dfLast = df.iloc[:-3, -1]

fig2 = px.pie(dfLast, values=dfLast.values, names=dfLast.index, title="Uranium production by country", height=800, width=800)
